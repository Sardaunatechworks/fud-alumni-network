...
import { 
  User, Mail, Lock, Bell, Shield, Camera, Save, Globe, BookOpen, Briefcase, ChevronRight, ExternalLink, Trash2, Calendar
} from 'lucide-react';
              ...
              {tabs.map((tab) => (
                <button
                  ...
                  className={`
                    ...
                    ${activeTab === tab.id 
                      ? 'bg-white shadow-xl shadow-indigo-900/5 ring-1 ring-slate-200' 
                      : 'hover:bg-white/50'}
                  `}
                >
                  <div className="flex items-center gap-4">
                    <div className={`
                      ...
                      ${activeTab === tab.id ? 'bg-indigo-900 text-white' : 'bg-slate-100 text-slate-500 group-hover:bg-slate-200'}
                    `}>
                      {tab.icon}
                    </div>
                    <div>
                  ...
                  <ChevronRight size={16} className={`transition-all ${activeTab === tab.id ? 'translate-x-0 text-indigo-900' : '-translate-x-2 opacity-0 text-slate-300'}`} />
                </button>
              ))}

              <div className="mt-8 rounded-2xl bg-indigo-900 p-6 text-white shadow-lg">
                <h4 className="text-sm font-bold">Need help?</h4>
                <p className="mt-1 text-[10px] text-indigo-200">Our support team is available 24/7 to assist you with your account.</p>
                      ...
                      <div className="flex flex-col items-center gap-8 sm:flex-row">
                        <div className="relative group">
                          <div className="h-28 w-28 overflow-hidden rounded-[2rem] ring-4 ring-slate-50 shadow-inner">
                            <img src="https://picsum.photos/seed/student/200/200" alt="Avatar" className="h-full w-full object-cover" referrerPolicy="no-referrer" />
                          </div>
                          <button className="absolute -right-2 -bottom-2 flex h-10 w-10 items-center justify-center rounded-2xl bg-indigo-900 text-white shadow-lg ring-4 ring-white transition-transform hover:scale-110">
                            <Camera size={18} />
                          </button>
                        </div>
                        <div className="flex-1 text-center sm:text-left">
                          ...
                          <div className="mt-4 flex flex-wrap justify-center gap-3 sm:justify-start">
                            <button className="rounded-xl bg-slate-100 px-4 py-2 text-xs font-bold text-slate-600 transition-all hover:bg-slate-200">
                              Upload New Photo
                            </button>
                            <button className="rounded-xl border border-red-100 px-4 py-2 text-xs font-bold text-red-500 transition-all hover:bg-red-50">
                              Remove
                            </button>
                          </div>
                        </div>
                      ...
                      <div className="grid grid-cols-1 gap-6 sm:grid-cols-2">
                        <div className="sm:col-span-2">
                          ...
                          <div className="relative">
                            ...
                            <input type="text" value={formData.fullName} onChange={(e) => setFormData({...formData, fullName: e.target.value})} className="w-full rounded-2xl border-transparent bg-slate-50 py-3.5 pr-4 pl-12 text-sm font-medium ring-1 ring-slate-100 placeholder:text-slate-400 focus:border-transparent focus:bg-white focus:ring-2 focus:ring-indigo-500/50 transition-all shadow-sm" />
                          </div>
                        </div>
                        <div>
                        ...
                        <div>
                          ...
                          <input type="text" value={formData.phone} onChange={(e) => setFormData({...formData, phone: e.target.value})} placeholder="e.g. +234 800 000 0000" className="w-full rounded-2xl border-transparent bg-slate-50 py-3.5 px-4 text-sm font-medium ring-1 ring-slate-100 placeholder:text-slate-400 focus:border-transparent focus:bg-white focus:ring-2 focus:ring-indigo-500/50 transition-all shadow-sm" />
                        </div>
                      </div>
                    </section>
                    ...
                    <section className="rounded-3xl bg-white p-8 shadow-sm ring-1 ring-slate-200">
                      ...
                      <div className="grid grid-cols-1 gap-6 sm:grid-cols-2">
                        <div>
                          ...
                          <input type="text" value={formData.faculty} onChange={(e) => setFormData({...formData, faculty: e.target.value})} className="w-full rounded-2xl border-transparent bg-slate-50 py-3.5 px-4 text-sm font-medium ring-1 ring-slate-100 placeholder:text-slate-400 focus:border-transparent focus:bg-white focus:ring-2 focus:ring-indigo-500/50 transition-all shadow-sm" />
                        </div>
                        <div>
                          <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1.5 ml-1">Department</label>
                          <input type="text" value={formData.department} onChange={(e) => setFormData({...formData, department: e.target.value})} className="w-full rounded-2xl border-transparent bg-slate-50 py-3.5 px-4 text-sm font-medium ring-1 ring-slate-100 placeholder:text-slate-400 focus:border-transparent focus:bg-white focus:ring-2 focus:ring-indigo-500/50 transition-all shadow-sm" />
                        </div>
                        <div className="sm:col-span-2">
                          ...
                          <div className="flex flex-wrap gap-2 mt-2">
                            {['Software Development', 'AI', 'Cybersecurity', 'Web Dev'].map(tag => (
                              <span key={tag} className="flex items-center gap-2 rounded-xl bg-indigo-50/80 backdrop-blur-sm px-3 py-1.5 text-xs font-bold text-indigo-700 ring-1 ring-indigo-100/50 shadow-sm transition-all hover:shadow-md hover:ring-indigo-200">
                                {tag} <button className="hover:text-indigo-900 transition-colors"><Trash2 size={12} /></button>
                              </span>
                            ))}
                            <button className="rounded-xl border border-dashed border-slate-300 bg-slate-50/50 px-3 py-1.5 text-xs font-bold text-slate-400 transition-all hover:border-indigo-300 hover:bg-indigo-50/50 hover:text-indigo-600">
                              + Add Interest
                            </button>
                          </div>
                        </div>
                      ...
                      <div className="space-y-6">
                        <div>
                          ...
                          <div className="relative">
                            ...
                            <input type="password" placeholder="••••••••" className="w-full rounded-2xl border-transparent bg-slate-50 py-3.5 pr-4 pl-12 text-sm font-medium ring-1 ring-slate-100 placeholder:text-slate-400 focus:border-transparent focus:bg-white focus:ring-2 focus:ring-indigo-500/50 transition-all shadow-sm" />
                          </div>
                        </div>
                        <div className="grid grid-cols-1 gap-6 sm:grid-cols-2">
                          <div>
                ...
                <div className="sticky bottom-6 z-10 flex items-center justify-between rounded-3xl bg-white/80 p-6 shadow-[0_-8px_30px_-15px_rgba(0,0,0,0.1)] ring-1 ring-slate-200/50 backdrop-blur-xl supports-[backdrop-filter]:bg-white/60">
                  ...
                  <div className="flex w-full items-center justify-end gap-3 sm:w-auto">
                    ...
                    <button 
                      onClick={handleSave}
                      disabled={isSaving}
                      className="flex min-w-[160px] items-center justify-center gap-2 rounded-xl bg-indigo-900 px-8 py-3 text-sm font-bold text-white shadow-lg shadow-indigo-900/20 transition-all hover:bg-indigo-800 disabled:opacity-70"
                    >
                      {isSaving ? (
                        <>
                          <motion.div 
